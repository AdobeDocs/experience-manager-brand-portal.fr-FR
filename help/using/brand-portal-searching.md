...
description: La fonctionnalité de recherche de Brand Portal vous permet de rechercher rapidement des ressources pertinentes à l’aide de l’omni-recherche. Les filtres de recherche vous aident à affiner davantage votre recherche. Enregistrez vos recherches en tant que collections dynamiques pour une utilisation ultérieure.
contentOwner: bdhar
...
1. Dans la barre d’outils, cliquez sur l’icône **[!UICONTROL Rechercher]** ou appuyez sur la touche **[!UICONTROL /]** (barre oblique) pour lancer Omnisearch.
...
1. Dans la zone de recherche, saisissez un mot-clé pour les ressources à rechercher.
   ...
   >[!NOTE]
   >
   >* Pour que des suggestions de recherche s’affichent, au moins 3 caractères sont requis dans Omnisearch.
   >* Lorsque vous recherchez des `mountain biking`, Omnisearch renvoie toutes les ressources des résultats de la recherche qui contiennent des `mountain` et des `biking` dans les champs de métadonnées (par exemple, `mountain` dans le champ `Title` et `biking` dans le champ `Description`). Les deux termes doivent être disponibles dans les champs de métadonnées pour s’afficher dans les résultats de la recherche. Cependant, Omnisearch renvoie la ressource dans les résultats de la recherche même si un seul des deux termes est disponible dans le champ de métadonnées Balises intelligentes. Supposons, par exemple, qu’une ressource ait `mountain` comme balise active, mais qu’elle manque de `biking` dans tout autre champ de métadonnées. Ensuite, vous recherchez `mountain biking`. Omnisearch renvoie toujours la ressource dans les résultats de la recherche. Ce workflow permet de s’assurer que les ressources présentant les balises appropriées ne sont pas manquantes.

1. Faites votre choix parmi les suggestions associées qui s’affichent dans la liste déroulante pour accéder rapidement aux ressources appropriées.

   ![](assets/assets-search-result.png)

   *Recherche de ressources à l’aide de l’omni-recherche*

Pour en savoir plus sur le comportement de la recherche avec les ressources balisées intelligentes, accédez à [comprendre les résultats et le comportement de la recherche](https://experienceleague.adobe.com/fr/docs/experience-manager-65/content/assets/using/search-assets).

## Recherche à l’aide des facettes du panneau Filtres {#search-using-facets-in-filters-panel}

Les facettes de recherche du panneau Filtres ajoutent une granularité à votre expérience de recherche et rendent la fonctionnalité de recherche plus efficace. Elles utilisent plusieurs dimensions (prédicats) qui vous permettent d’effectuer des recherches complexes. Vous pouvez facilement descendre dans la hiérarchie jusqu’au niveau de détail souhaité pour effectuer une recherche plus précise.

Par exemple, si vous recherchez une image, vous pouvez choisir si vous souhaitez une bitmap ou une image vectorielle. Vous pouvez affiner davantage la portée de la recherche en spécifiant le type MIME de l’image dans la facette de recherche Type de fichier. De même, lors de la recherche de documents, vous pouvez spécifier le format, par exemple PDF ou MS® Word.

![Panneau Filtres dans Brand Portal](assets/file-type-search.png "Panneau Filtres dans Brand Portal")

Le panneau **[!UICONTROL Filtres]** comprend quelques facettes standard, telles que- **[!UICONTROL Navigateur de chemin d’accès]**, **[!UICONTROL Type de fichier]**, **[!UICONTROL Taille de fichier]**, **[!UICONTROL Statut]** et **[!UICONTROL Orientation]**.
Cependant, vous pouvez [ajouter des facettes de recherche personnalisées](../using/brand-portal-search-facets.md) ou supprimer des facettes spécifiques du panneau **[!UICONTROL Filtres]**. Modifiez simplement les prédicats dans le formulaire de recherche sous-jacent. Consultez la liste des [prédicats de recherche disponibles et utilisables sur Brand Portal](../using/brand-portal-search-facets.md#list-of-search-predicates).
...
Par exemple, utilisez les filtres standard suivants :
   ...
   * **[!UICONTROL Explorateur de chemins d’accès]** pour rechercher des ressources dans un répertoire donné. Le chemin de recherche par défaut du prédicat pour l’explorateur de chemins d’accès est `/content/dam/mac/<tenant-id>/`. Il peut être configuré en modifiant le formulaire de recherche par défaut.

   >[!NOTE]
   >
   >Pour les utilisateurs et utilisatrices ne disposant pas de droits d’administration, le [!UICONTROL navigateur de chemins d’accès] du panneau [!UICONTROL Filtre] affiche uniquement la structure de contenu des dossiers (et de leurs dossiers ancêtres) partagés avec eux.\
   ...
   * Le **[!UICONTROL Prédicat de propriété]** s’il est utilisé dans le formulaire de recherche, vous permet de rechercher des ressources correspondant à une propriété de métadonnées à laquelle le prédicat est mappé.\
...
Pour permettre la recherche de ressources à l’aide d’expressions partielles dans le prédicat de propriété, cochez la case **[!UICONTROL Recherche partielle]** dans le formulaire de recherche. Cette méthode vous permet de rechercher les ressources souhaitées même si vous ne spécifiez pas les mots ou expressions exacts utilisés dans les métadonnées de la ressource.
     ...
     Vous pouvez :
      * Spécifiez un mot apparaissant dans l’expression recherchée dans la facette du panneau Filtres. Par exemple, si vous recherchez le terme **climb** (et que le prédicat de propriété est mappé à la propriété `dc:title`), toutes les ressources contenant le mot **climb** dans leur titre sont renvoyées.
...
Vous pouvez autoriser une recherche non sensible à la casse dans le prédicat de propriété. Activez simplement la case **[!UICONTROL Ignorer la casse]** dans le formulaire de recherche. Par défaut, la recherche de texte dans le prédicat de propriété est sensible à la casse.
   ...
   Résultat de recherche de ressources avec le nombre de résultats de recherche.
...
Vous pouvez enregistrer les paramètres de recherche sous la forme d’une collection dynamique afin de pouvoir répéter rapidement la même recherche sans avoir à redéfinir les mêmes paramètres ultérieurement. Cependant, vous ne pouvez pas appliquer de filtres de recherche dans une collection.

Pour enregistrer les paramètres de recherche comme collection dynamique :

1. Cliquez sur **[!UICONTROL Enregistrer la collection dynamique]** et attribuez un nom à la collection dynamique.
   ...
   >[!NOTE]
   >
   >Vous pouvez empêcher des utilisateurs non administrateurs de rendre les collections dynamiques publiques afin d’éviter qu’un grand nombre de collections dynamiques publiques ne soient créées par des utilisateurs non administrateurs sur le Brand Portal de l’organisation. Les organisations peuvent désactiver la configuration **[!UICONTROL Autoriser la création de collections dynamiques publiques]** dans les paramètres **[!UICONTROL Général]** disponibles dans le panneau des outils d’administration.
